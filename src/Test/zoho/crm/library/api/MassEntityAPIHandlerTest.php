...
class MassEntityAPIHandlerTest
{
	...
	public function testCreateRecords()
	{
		...
		foreach ($moduleList as $apiName=>$moduleName)
		{
			...
			}catch (ZCRM\Exception $e)
			{
				...
				Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\Module('.$moduleName.')','create',$e->getMessage(),$e->getTraceAsString(),'failure',($endTime-$startTime));
			}
		}
	}
	...
	public function testUpdateRecords()
	{
		...
		{
			$endTime=$endTime==0?microtime(true)*1000:$endTime;
			Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\Module('.$moduleName.')','update',$e->getMessage(),$e->getTraceAsString(),'failure',($endTime-$startTime));
		}
	}
	...
	public function testUpsertRecords()
	{
		...
		foreach ($moduleList as $apiName=>$moduleName)
		{
			...
			{
				$endTime=$endTime==0?microtime(true)*1000:$endTime;
				Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\Module('.$moduleName.')','upsertRecords',$e->getMessage(),$e->getTraceAsString(),'failure',($endTime-$startTime));
			}
		}
	}
	...
	public function validateGetDeletedRecordsResponse($apiName,$startTime,$endTime)
	{
		...
		foreach ($types as $type)
		{
			try {
				Main::incrementTotalCount();
				$startTime=microtime(true)*1000;
				$moduleIns=ZCRM\Module::getInstance($apiName);
				...
				Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\Module('.$apiName.")",$methodName,"Deleted Records fetched successfully","ID List:".json_encode($deletedIdList),'success',($endTime-$startTime));
				unset($deletedIdList);
			}catch (ZCRM\Exception $e)
			{
				$endTime=$endTime==0?microtime(true)*1000:$endTime;
				...
				{
					Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\Module('.$apiName.")",$methodName,"Deleted Records fetched successfully","No Records Exist",'success',($endTime-$startTime));
					continue;
				}
	...
	public function validateDeleteRecordsResponse($moduleAPIName,$startTime,$endTime)
	{
		try{
			...
			Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\Module('.$moduleAPIName.")",'deleteRecords()',"Records deleted Successfully","ID List:".json_encode($deletedIdList),'success',($endTime-$startTime));
			unset($deletedIdList);
		}
		catch (ZCRM\Exception $e)
		{
	...
	public function validateGetRecordsResponse($moduleAPIName,$moduleName,$startTime,$endTime)
	{
		try {
			...
			Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\Module('.$moduleAPIName.")",'getRecords()',"Bulk Records fetched successfully","Fetch Count::".$fetchCount,'success',($endTime-$startTime));
			unset($fetchedIds);
		}
		catch (ZCRM\Exception $e)
		{
	...
	public function updateRecordFieldsAndValidate($moduleAPIName,$startTime,$endTime)
	{
		try
		{
			...
			foreach ($layoutIds as $layoutId)
			{
				...
				{
					$endTime=$endTime==0?microtime(true)*1000:$endTime;
					Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),"ZCRM\Module(".$moduleAPIName.")",'updateRecords()',"Records Updated Successfully","EntityId List::".json_encode(self::$moduleApiNameVsEntityIds[$moduleAPIName]),'success',($endTime-$startTime));
				}
			}
		}
		catch (ZCRM\Exception $e)
	...
	public function setRecordFieldsAndValidate($moduleAPIName,$startTime,$endTime,$method)
	{
		try{
			...
			foreach ($layoutIds as $layoutId)
			{
				...
				if($method=="upsert")
				{
					$zcrmrecord=ZCRM\Record::getInstance($moduleAPIName, self::$moduleApiNameVsEntityIds[$moduleAPIName][0]);
					...
					{
						$endTime=$endTime==0?microtime(true)*1000:$endTime;
						Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\Module('.$moduleAPIName.")",'upsertRecords',"Records upsert completed Successfully","Entity Id List::".json_encode($successIds),'success',($endTime-$startTime));
					}
				}
				else 
				{
					...
					{
						$endTime=$endTime==0?microtime(true)*1000:$endTime;
						Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\Module('.$moduleAPIName.")",'createRecords',"Records Created Successfully","Entity Id List::".json_encode(self::$moduleApiNameVsEntityIds[$moduleAPIName]),'success',($endTime-$startTime));
					}
				}
				unset($recordsArray);
			}
