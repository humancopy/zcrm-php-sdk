...
class MetaDataAPIHandlerTest
{
	...
	public function testGetAllModules()
	{
		try{
			...
			foreach ($moduleArr as $module)
			{
				...
				{
					throw new ZCRM\Exception("Some fields data is not fetched");
				}
				else if($module->getModuleName()!='Feeds')
				{
					if(sizeof($module->getAllProfiles())==0)
					{
						throw new ZCRM\Exception("Profiles not set in the module object");
					}
					else
					{
						...
						foreach ($profiles as $profile)
						{
							...
							{
								throw new ZCRM\Exception("Profile Data is not set properly in the module object");
							}
						}
					}
				}
			...
			Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getAllModules',null,null,'success',($endTime-$startTime));
			
		}catch (ZCRM\Exception $e)
		{
			$endTime=$endTime==0?microtime(true)*1000:$endTime;
			Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getAllModules',$e->getMessage(),$e->getExceptionDetails(),'failure',($endTime-$startTime));
		}
	...
	public function testGetModule()
	{
		...
		{
			Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule','Invalid Response','Module List is null or empty','failure',0);
			return;
		}
		foreach(self::$moduleList as $apiName=>$moduleName)
		{
			try{
				...
				{
					Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Response status code is not as expected('.$moduleResponse->getHttpStatusCode().')','failure',($endTime-$startTime));
					continue;
				}
				...
				{
					Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Module fields data not fetched properly','failure',($endTime-$startTime));
					continue;
				}
				if($zcrmModule->getRelatedLists()!=null)
				{
					...
					foreach ($relatedLists as $relatedList)
					{
						...
						{
							Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Module Related List details not fetched properly','failure',($endTime-$startTime));
							break;
						}
					}
				}
				...
				foreach ($accessibleProfiles as $profile)
				{
					...
					{
						Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Module accessible profiles data not fetched properly','failure',($endTime-$startTime));
						break;
					}
				}
				if(!is_array($zcrmModule->getProperties()) && !$moduleName=='Approvals')
				{
					Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Module properties not fetched properly','failure',($endTime-$startTime));
					continue;
				}
				...
				{
					Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Module Business card fields not fetched properly','failure',($endTime-$startTime));
					continue;
				}
				...
				foreach ($rawFieldArray as $field)
				{
					...
					{
						Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Module Field details not fetched properly','failure',($endTime-$startTime));
						break;
					}
					$fieldDetails=array();
				...
				if($zcrmModule->getLayouts()!=null)
				{
					...
					foreach ($layouts as $layout)
					{
						...
						{
							Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Module Layout details not fetched properly','failure',($endTime-$startTime));
							break;
						}
						...
						foreach ($layoutProfiles as $profile)
						{
							...
							{
								Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Module layout accessible profiles data not fetched properly','failure',($endTime-$startTime));
								break;
							}
						}
						...
						if(sizeof($convertMapping)>0)
						{
							...
							{
								Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Lead convert mapping for Accounts not fetched properly','failure',($endTime-$startTime));
								break;
							}
							else if(!is_string($dealMapIns->getName()) || !is_long($dealMapIns->getId()))
							{
								Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Lead convert mapping for Deals not fetched properly','failure',($endTime-$startTime));
								break;
							}
							else if(!is_string($conMapIns->getName()) || !is_long($conMapIns->getId()))
							{
								Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Lead convert mapping for Contacts not fetched properly','failure',($endTime-$startTime));
								break;
							}
						}
						...
						foreach ($sections as $section)
						{
							...
							{
								Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Module layout section data not fetched properly','failure',($endTime-$startTime));
								break;
							}
							...
							foreach ($fields as $field)
							{
								...
								{
									Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$moduleName.')','Invalid Response','Module layout section fields data not fetched properly','failure',($endTime-$startTime));
									break;
								}
				...
				Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$apiName.')',null,null,'success',($endTime-$startTime));
			}catch (ZCRM\Exception $e)
			{
				$endTime=$endTime==0?microtime(true)*1000:$endTime;
				Helper::writeToFile(self::$filePointer,Main::getCurrentCount(),'ZCRM\RestClient','getModule('.$apiName.')',$e->getMessage(),$e->getTraceAsString(),'failure',($endTime-$startTime));
			}
		}
	}
